...
class SwitchApi:
    ...
    def get_switch_layer_three_info(self, router_id=None):
        response = requests.get(
            self.base_url_router + ("000000000000000" + router_id) if router_id is not None else "all")
    ...
    def set__address(self, switch_id, address):
        response = requests.post(self.base_url_router + "000000000000000" + switch_id, json=dict([("address", address)]))
    ...
    def set__default_route(self, switch_id, gateway):
        response = requests.post(self.base_url_router + "000000000000000" + switch_id, json=dict([("gateway", gateway)]))
        set_address_response = SwitchApi.headers(response)
        set_address_response['data'] = response.json()
